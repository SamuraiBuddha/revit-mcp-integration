...
Converts Revit API documentation HTML files to structured JSON for AI training
"""
...
class RevitAPIParser:
    """Parser for converting Revit API HTML documentation to JSON format"""
    ...
    def _generate_description_answer(self, data: Dict[str, Any]) -> str:
        ...
        if data['inheritance']:
            parts.append(f"It inherits from: {' → '.join(data['inheritance'])}")
    ...
    def batch_process(self, input_dir: str) -> None:
        ...
        raw_output = self.output_dir / 'revit_api_raw.json'
        ...
        # Save training data
        training_output = self.output_dir / 'revit_api_training.json'
        ...
        stats_output = self.output_dir / 'processing_stats.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parse Revit API HTML documentation to JSON')
